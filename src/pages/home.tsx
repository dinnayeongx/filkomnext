    ...
    <div className="bg-primary bg-opacity-5 py-16 flex flex-col items-start min-h-[150vh] relative z-0">
      <div className="flex gap-12 px-20">
        ...
        <div className="flex flex-col gap-16 items-start justify-center z-20">
          ...
          <p className="text-xl w-[560px]">Temukan karier teknologi yang paling cocok dengan kepribadianmu! <br /> FILKOMNEXT membantu kamu mengenali potensi diri menggunakan model O*NET dan mencocokkannya dengan berbagai jalur karier di bidang teknologi seperti Data Science, Cybersecurity, UI/UX, Software Engineering, dan lainnya.</p>
          <Button variant="primary" onClick={() => window.location.href = "/quiz"}>Mulai Kuis</Button>
        </div>
      ...
      <div className="bg-bg-hero mt-8 pb-36 w-full flex justify-center items-center bg-no-repeat bg-contain h-[900px] gap-16 z-20 px-28">
        ...
        <div ref={carouselRef} className="flex gap-12 overflow-hidden overflow-x-hidden no-scrollbar scroll-smooth p-16">
      ...
      <div className="px-28 gap-12 flex flex-col mb-52">
        ...
        <p className="text-2xl">Bagaimana Kepribadianmu Mempengaruhi Pilihan Karier di Bidang Teknologi? <br /> <br /> Setiap individu memiliki kekuatan unik dari pemecah masalah logis hingga desainer yang empatik. <br /> FILKOMNEXT menghubungkan hasil asesmen kepribadian berbasis O*NET dengan jalur karier teknologi yang paling sesuai.</p>
        <Button variant="primary" onClick={() => window.location.href = "/article"}>Lihat Selengkapnya</Button>
      </div>
    </div>
  )
};
